...
def validate_case_lifecycle(
    ...
    for required in REQUIRED_LIFECYCLE:
        try:
            ...
            raise ValueError(
                f"trace for {case_id} is missing ordered lifecycle event {required!r}"
            ) from exc
    consumed_refs = {
        ref
        for event in events
    ...
    unlinked = sorted(set(output["evidence_refs"]) - consumed_refs)
    if unlinked:
        raise ValueError(
            f"outputs/{case_id}.json contains evidence absent from tool_result_consumed: "
            f"{unlinked}"
        )
...
def validate_artifacts(
    ...
    for case_id, output in outputs.items():
        validate_case_lifecycle(case_id, output, events_by_case[case_id])
